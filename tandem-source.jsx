/* Tandem — shared household organizer. Phase 1.
   Design authority: Tandem Mockups turn 8 (Olive & Terracotta). */
...
function loadState() {
  try { const s = JSON.parse(localStorage.getItem(LS_STATE)); if (s && s.household) return s; } catch (e) {}
  return { household: defaultHousehold(), tasks: seedTasks(), nudges: [] };
}
function seedTasks() {
  const t = todayStr();
  return [
      ...
      cur.tasks.forEach((t) => {
        ...
        if (new Date(t.reminder.at) <= new Date()) {
          if (typeof Notification !== "undefined" && Notification.permission === "granted")
            new Notification("Tandem", { body: t.title + " — " + fmtDue(t) });
...
function Onboard({ members, onPick }) {
  return (
    <div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100vh", background: C.bg, fontFamily: FONT, display: "flex", flexDirection: "column", justifyContent: "center", padding: 28 }}>
      <div style={{ fontSize: 26, fontWeight: 800, color: C.ink, letterSpacing: "-.02em" }}>Tandem</div>
      ...
      {draft && (<>
        <div style={{ ...card(22), padding: "16px 18px", marginTop: 11 }}>
          ...
          <div style={{ display: "flex", gap: 7, marginTop: 12, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 11.5, fontWeight: 700, color: C.oliveDark, background: C.oliveSoft, borderRadius: 999, padding: "6px 12px", whiteSpace: "nowrap" }}>saved to Tandem</span>
...
function CalendarView({ state, me, other, nav, goTab, tab }) {
  ...
  const anyOutlook = state.household.members.some((m) => m.outlook?.connected);
  const selDate = new Date(sel + "T00:00:00");
  const selTasks = tasksOn(sel);
  return (<>
    <div style={{ display: "flex", alignItems: "center", padding: "20px 20px 8px" }}>
      <H1>Calendar</H1>
      <span onClick={() => nav("sharing")} style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 11, fontWeight: 700, color: anyOutlook ? C.oliveDark : C.mut, background: anyOutlook ? C.oliveSoft : C.track, borderRadius: 999, padding: "5px 11px", whiteSpace: "nowrap", cursor: "pointer" }}>
        <i style={{ width: 6, height: 6, borderRadius: "50%", background: anyOutlook ? C.olive : C.faint, display: "block" }} />Outlook · {anyOutlook ? "connected" : "not yet"}
      </span>
    </div>
    <TabStrip tab={tab} goTab={goTab} />
    <div style={{ padding: "6px 20px 18px" }}>
      <div style={{ display: "flex", gap: 5, marginTop: 2, background: "#fff", borderRadius: 20, padding: "8px 6px", boxShadow: shadow }}>
        {days.map((d) => {
          const ds = todayStr(d); const selD = ds === sel; const dots = tasksOn(ds);
          return (
            <div key={ds} onClick={() => setSel(ds)} style={{ flex: 1, textAlign: "center", padding: "8px 0", borderRadius: 14, background: selD ? C.ink : "transparent", cursor: "pointer" }}>
              ...
              <div style={{ display: "flex", gap: 2, justifyContent: "center", marginTop: 4, height: 4 }}>
                {dots.slice(0, 3).map((_, i) => <i key={i} style={{ width: 4, height: 4, borderRadius: "50%", background: selD ? "#a3ad7a" : C.olive, display: "block" }} />)}
              </div>
            </div>
          );
        })}
      ...
      {selTasks.length === 0 && <div style={{ fontSize: 13, color: C.mut, fontWeight: 600, padding: "6px 2px" }}>Nothing on this day yet — a quiet one.</div>}
      {selTasks.map((t) => {
        ...
        return (
          <div key={t.id} onClick={() => nav("task", { id: t.id })} style={{ display: "flex", gap: 12, background: C.oliveSoft, borderRadius: 20, padding: "13px 15px", marginBottom: 9, cursor: "pointer", opacity: t.status === "done" ? 0.7 : 1 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11.5, fontWeight: 600, color: C.oliveDark, marginTop: 2 }}>{t.daily ? "daily" : (t.dueTime || "any time")} · {together ? "together" : ""}{t.listItems?.length ? " · list attached" : ""}{!together && !t.listItems?.length ? "in Tandem" : ""}</div>
            </div>
            {together
      ...
      {!anyOutlook && (
        <div style={{ fontSize: 11.5, fontWeight: 600, color: C.mut, lineHeight: 1.5, margin: "14px 2px 0" }}>
          Outlook isn't wired up yet — once it is, tasks land in each person's calendar and their events can show here. Set per person in <span onClick={() => nav("sharing")} style={{ color: C.olive, cursor: "pointer" }}>Sharing & access</span>.
        </div>
      )}
    </div>
  </>);
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11.5, fontWeight: 600, color: m.outlook?.connected ? C.oliveDark : C.mut, marginTop: 2 }}>
                ...
                {m.outlook?.connected ? "Outlook connected · " + m.outlook.email : "Outlook — coming in the next phase"}
              </div>
            </div>
          </div>
...
function Settings({ state, me, nav, syncStatus, setSyncStatus }) {
  ...
  const saveAnth = () => { saveKeys({ ...loadKeys(), anthropic: anth.trim() }); setMsg("Claude key saved on this device."); };
  const connect = async () => {
    saveKeys({ ...loadKeys(), supabaseUrl: su.trim(), supabaseAnon: sk.trim() });
    sb = null;
  ...
  return (<>
    ...
    <div style={{ padding: "0 20px 24px" }}>
      ...
      <div style={{ ...card(22), padding: "16px 17px", marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 11.5, fontWeight: 600, color: C.mut, marginTop: 3, lineHeight: 1.5 }}>Lets Tandem draft tasks from photos, read paper notes and verify after-photos. Stored only on this device.</div>
      ...
      <div style={{ fontSize: 11, color: C.faint, fontWeight: 600, textAlign: "center", marginTop: 22 }}>Tandem · phase one · Outlook & pocket pushes to follow</div>
    </div>
  </>);
}
